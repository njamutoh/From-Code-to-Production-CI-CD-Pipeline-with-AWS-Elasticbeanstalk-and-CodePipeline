# NovaMed Appointment Booking System
...
**NovaMed Clinic Network** is a fast-growing Canadian healthcare company operating **12 clinics across Ontario and Quebec**, serving over **47,000 patients annually**. As of 2024, their appointment booking process ran entirely on phone calls and manually maintained spreadsheets — a system visibly breaking under growth pressure.
...
Beyond the operational problems, NovaMed's 4-person engineering team had no standardized release process:
...
**2. A production-grade CI/CD pipeline** — every code change is automatically tested, deployed to staging, validated, and held at a manual approval gate before the Medical Director authorises a production release. No code reaches patients without being tested and signed off.
...
**The problem it solves:** NovaMed's engineers are application developers, not infrastructure engineers. Provisioning EC2 instances, configuring Auto Scaling groups, setting up ALBs, patching OS packages — none of that delivers patient value.

**Why Beanstalk specifically:**
- Handles provisioning, load balancing, auto-scaling, and health monitoring automatically
- Supports multiple named environments (`novamed-staging`, `novamed-preprod`, `novamed-production`) — essential for the multi-stage pipeline
...
**Why needed:** `appointments.novamed.ca` with valid TLS is a baseline requirement for any patient-facing healthcare application — for trust, usability, and regulatory optics under PIPEDA (Canada's federal privacy law).
...
**Why needed:** NovaMed's clinics span from Windsor to Montreal. Without a CDN, all static assets are served from a single `ca-central-1` origin. CloudFront caches at Canadian PoPs, reducing latency for clinic staff, and absorbs traffic spikes during high-demand periods (flu season, Monday mornings).
...
User / Browser
      |
   Route 53              (planned — custom domain: appointments.novamed.ca)
      |
...
novamed-staging               novamed-production
novamed-preprod
      |                             |
      +─────────────────────────────+
                    |
...
GitHub push to main
  ...
  → [3] Deploy Staging  Beanstalk novamed-staging             ← Continuous Deployment
  → [4] Smoke Test      CodeBuild — tests against live staging URL
  → [5] Pre-Production  Beanstalk novamed-preprod             ← Continuous Delivery
  → [6] Approval        SNS email to Medical Director — approve or reject
  → [7] Production      Beanstalk novamed-production (only if approved)
```
...
| 3 — Deploy Staging | Elastic Beanstalk | Auto-deploys to `novamed-staging` if all tests pass |
...
| 5 — Pre-Production | Elastic Beanstalk | Deploys to `novamed-preprod` for final validation |
| 6 — Approval | SNS | Email sent to Medical Director — one-click approve or reject |
| 7 — Production | Elastic Beanstalk | Deploys to `novamed-production` only after approval |
...
```bash
# Clone the repo
git clone https://github.com/njamutoh/Production-Ready-Three-Tier-Containerized-Application-on-AWS.git
cd novamed-appointments
...
eb init novamed-appointments --region ca-central-1 --platform python-3.11

# Create the three environments
eb create novamed-staging
eb create novamed-preprod
eb create novamed-production

# Manual deploy (CodePipeline handles this automatically once the pipeline is set up)
eb deploy novamed-production
```
